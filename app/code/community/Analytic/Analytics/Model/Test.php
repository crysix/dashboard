...
class Analytic_Analytics_Model_Test extends Mage_Core_Model_Abstract
{
    ...
    private function addDomains()
    {
        ...
        $this->writeConnection->beginTransaction();
        for ($i = 0; $i <= 100; $i++) {
            $this->writeConnection->insert('analytic_ref_domain', array('domain_name' => 'www.test' . $i . '.com'));
        }
        $this->writeConnection->commit();
    }
    ...
    public function run($customersCount = 0, $importProducts = false, $isAddDomain = false, $isAddUrls = false)
    {
        ...
        $analyticTable = "analytic_data";
        $analyticProductTable = "analytic_product";
        ...
        for ($day = 1; $day <= 365; $day++) {
            for ($time = 0; $time <= 23; $time++) {
                ...
                for ($i = 0; $i < $this->customerDailyActivityCount; $i++) {
                    ...
                    $this->writeConnection->insert($analyticTable, $values);

                    $analyticId = $this->writeConnection->lastInsertId();
                    $this->addCustomerActivity($analyticId);
                    $this->addShopActivity($analyticId);
                    // add products activity
                    if ($productCount) {
                        $this->generateProductsActivity($productCount, $analyticId, $analyticProductTable);
                    }

                    $this->generateReferals($analyticId);
                    $this->generateUserPageActivity($analyticId);
                    $this->generateCategoriesViewsActivity($analyticId);
                    $this->generateSearchesActivity($analyticId);
    ...
    private function addCustomerActivity($analyticId)
    {
        $this->writeConnection->insert('analytic_customer', array(
    ...
    private function addShopActivity($analyticId)
    {
        $this->writeConnection->insert('analytic_shop', array(
            'analytic_id' => $analyticId,
     ...
     * @param string $analyticProductTable
     */
    private function generateProductsActivity($productCount, $analyticId, $analyticProductTable)
    {
        ...
        for ($j = 0; $j < 4; $j++) {
            ...
            if (!$inserts) {
                $inserts = array(
                    'q' => sprintf('INSERT INTO `%s` (`%s`) VALUES ', $analyticProductTable, implode('`,`', array_keys($values))),
     ...
     * @param int $analyticId
     */
    private function generateReferals($analyticId)
    {
        //echo '----- add referal' . PHP_EOL;
        $this->writeConnection->insert('analytic_refer', array(
            'analytic_id' => $analyticId,
            'ref_id' => rand(1, 100),
            'ref_count' => 1
        ));
     ...
     * @param int $analyticId
     */
    private function generateUserPageActivity($analyticId)
    {
        //echo '----- add user page activity' . PHP_EOL;
        $this->writeConnection->insert('analytic_page', array(
            'analytic_id' => $analyticId,
            'view_count' => rand(1, 100)
        ));
        /*$maxPagesForSession = rand(1, 100);
        for ($i = 1; $i <= $maxPagesForSession; $i++) {
            $this->writeConnection->insert('analytic_pages', array(
                'analytic_id' => $analyticId,
                'url_id' => rand(1, 1000),
                'view_count' => rand(1, 100)
            ));
     ...
     * @param int $analyticId
     */
    private function generateCategoriesViewsActivity($analyticId)
    {
        //echo '----- add user categories view activity' . PHP_EOL;
        $maxPagesForSession = rand(1, 10);
        for ($i = 1; $i <= $maxPagesForSession; $i++) {
            $this->writeConnection->insert('analytic_category', array(
                'analytic_id' => $analyticId,
     ...
     * @param int $analyticId
     */
    private function generateSearchesActivity($analyticId)
    {
        //echo '----- add user searches' . PHP_EOL;
        $maxPagesForSession = rand(1, 10);
        for ($i = 1; $i <= $maxPagesForSession; $i++) {
            $this->writeConnection->insert('analytic_search', array(
                'analytic_id' => $analyticId,
    ...
    /**
     * check for partition of main analytic data table
     *
     * @param DateTime $currentDateTime
     */
    private function checkDataPartition($currentDateTime)
    {
        $maxDateTime = $this->readConnection->fetchOne('select MAX(date_time) from analytic_date_partition');
        ...
        if ($currentDateTime->diff($maxDateObject)->format('%R') != '+') {
            ...
            $this->writeConnection->insert('analytic_date_partition', array('date_time' => $maxDateObject->format('Y-m-d H:i:s')));
            $this->writeConnection->exec('ALTER TABLE analytic_data
                ADD PARTITION (
