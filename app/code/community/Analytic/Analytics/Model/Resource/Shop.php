...
class Analytic_Analytics_Model_Resource_Shop extends Analytic_Analytics_Model_Resource_Base
{
    /**
    ...
    private function getGroupData($fieldName, $dateFrom, $dateTo, $groupType, Mage_Eav_Model_Entity_Attribute_Abstract $attribute, $storeId = null, $limit = null, $whereValues = array())
    {
        ...
        $select = $adapter->select()
            ->from(
            array('analytic_shop' => $this->getPeriodTable('analytic/shop')),
            'addr.value AS group_value, SUM(analytic_shop.' . $fieldName . ') as count'
        )
            ->join(
            array('analytic' => $this->getPeriodTable('analytic/data')),
            'analytic.id = analytic_shop.analytic_id',
            array()
        )
            ->join(
