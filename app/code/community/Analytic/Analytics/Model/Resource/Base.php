...
abstract class Analytic_Analytics_Model_Resource_Base extends Mage_Reports_Model_Resource_Report_Abstract
{
    ...
    protected function getGroupPeriod($dateFrom, $dateTo)
    {
        ...
        $periodExpr = $adapter->getDatePartSql(
            $this->getStoreTZOffsetQuery(
                array('analytic' => $this->getTable('analytic/data')),
                'analytic.period', $dateFrom, $dateTo
            )
        );
        $groupByPeriod = $periodExpr->__toString();
        if ($this->getPlotType() === "monthly") {
            $dateGroup = 'CONCAT_WS("-", EXTRACT(YEAR FROM analytic.period), EXTRACT(MONTH FROM analytic.period), "01")';
            ...
            $dateGroup = 'analytic.period + INTERVAL 1 HOUR';
            $groupBy = 'analytic.period';
        }
    ...
    {
        if (Mage::helper('analytic_dashboard')->canUsePartitions() && $this->isPartitioningAvailable()) {
            $writeAdapter = $this->_getWriteAdapter();
            $readAdapter = $this->_getReadAdapter();

            $select = $readAdapter->select()
                ->from($this->getTable('analytic/date_partition'), array(
            ...
            if ($result && $result['max_date_time']) {
                ...
                $query = "ALTER TABLE `{$this->getTable('analytic/data')}`"
                ...
                $writeAdapter->insert(
                    $this->getTable('analytic/date_partition'),
            ...
            if ($datesDiff >= 0) {
                ...
                for ($i = 0; $i <= $monthCount; $i++) {
                    $maxDateObject->add(
                        Analytic_Analytics_Model_DataAggregation::PARTITION_INCREMENT_PERIOD,
                        Analytic_Analytics_Model_DataAggregation::PARTITION_INCREMENT_PART
                    );
                    $writeAdapter->insert(
                        $this->getTable('analytic/date_partition'),
                        array('date_time' => $maxDateObject->get('y-MM-dd 00:00:00'))
    ...
    /**
     * check for partition of main daily analytic data table
     *
     * @param Zend_Date $currentDateTime
     */
    public function checkDailyDataPartition(Zend_Date $currentDateTime)
    {
        if (Mage::helper('analytic_dashboard')->canUsePartitions() && $this->isPartitioningAvailable()) {
            ...
            $select = $readAdapter->select()
                ->from($this->getTable('analytic/date_partition_daily'), array('max_date_time' => 'MAX(date_time)'));
            $maxDateTime = $readAdapter->fetchOne($select);
            if($maxDateTime) {
                $maxDateObject = new Zend_Date($maxDateTime);
            } else {
                $maxDate = $currentDateTime->get('y');
                $query = "ALTER TABLE `{$this->getTable('analytic/data_daily')}`"
                    . " PARTITION BY RANGE(TO_DAYS(period)) ("
                    . " PARTITION p_{$maxDate} VALUES LESS THAN(TO_DAYS('{$maxDate}-12-31 23:59:59'))"
                    . ")";
                $writeAdapter->exec($query);
                $writeAdapter->insert(
                    $this->getTable('analytic/date_partition_daily'),
            ...
            if ($years > 0) {
                for ($i = 1; $i <= $years; $i++) {
                    $maxDateObject->add(
                        Analytic_Analytics_Model_DailyAggregation::PARTITION_INCREMENT_PERIOD,
                        Analytic_Analytics_Model_DailyAggregation::PARTITION_INCREMENT_PART
                    );
                    $writeAdapter->insert(
                        $this->getTable('analytic/date_partition_daily'),
    ...
    public function checkIfAggregationRun()
    {
        return $this->_getReadAdapter()->select()
            ...
            ->where('flag.flag_code = "analytic_aggregation"')
            ->query()->fetchColumn();
    }
    ...
    public function setAggregationState($state = false)
    {
        ...
        if ((int)$flagId) {
            ...
            $this->_getWriteAdapter()->insert(
                ...
                array(
                     'flag_data' => (int)$state,
                     'flag_code' => 'analytic_aggregation'
                )
            );
        }
    }
    ...
    protected function _prepareDataSelect($fieldName, $dateFrom, $dateTo, $storeId = null, $limit = null, $sort = null, $whereValues = array(), $aggregator = self::SUM_AGGREGATOR)
    {
        ...
        $select = $adapter->select()
            ->from(
                array('analytic_metric' => $this->getPeriodTable($this->_mainTable)),
                array(
                    'period_grouped' => new Zend_Db_Expr($dateFormat['dateGroup']),
                    'count' => new Zend_Db_Expr("$aggregator($fieldName)"),
                    'period_tz' => new Zend_Db_Expr("CONVERT_TZ(`analytic`.`period`,'+0:00','{$timezone}')"),
                )
            )
            ->join(
                array('analytic' => $this->getPeriodTable('analytic/data')),
                'analytic.id = analytic_metric.analytic_id',
                array()
            )
    ...
    protected function _prepareCountSelect($fieldName, $dateFrom, $dateTo, $storeId = null, $aggregator = self::SUM_AGGREGATOR)
    {
        ...
        $select = $adapter->select()
            ...
            ->join(
                array('analytic_metric' => $this->getPeriodTable($this->_mainTable)),
                'analytic_metric.analytic_id = analytic.id',
                array()
            )
            ->where('analytic.period > :dateFrom')
            ->where('analytic.period <= :dateTo');
