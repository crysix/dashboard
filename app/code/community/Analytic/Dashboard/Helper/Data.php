...
class Analytic_Dashboard_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    const XML_CURRENT_STORE_TIMEZONE = 'analytic_dashboard/settings/current_timezone';

    const XML_PATH_MIN_DATE = 'analytic_dashboard/general/min_date';
    const XML_PATH_PARTITIONING = 'analytic_dashboard/general/partitioning';
    ...
    public function getDefaultDashboardId()
    {
        ...
        $dashboardId = Mage::getModel("analytic_dashboard/user")->getDefaultDashboardId($userId);
    ...
    /**
     * Check if any analytic data is collected
     *
     * @return boolean
     */
    public function hasAnalyticsDataForPeriod()
    {
        /* @var $resource Analytic_Analytics_Model_Resource_DataAggregation */
        $resource   = Mage::getResourceModel("analytic/dataAggregation");
    ...
    {
        return Mage::getModel("analytic/cronJob")->getCollection()->count() > 0;
    }
    ...
    public function checkTimezoneChange()
    {
        ...
        if ($this->getAggregationType() === 'daily') {
            if (($savedTimezone !== $lastTimezone) &&
                !Mage::getResourceModel("analytic/dailyAggregation")->checkIfCronComplete()) {
                if (!Mage::getResourceModel("analytic/dailyAggregation")
                    ->getIfDateAgrregated($this->getDateFrom(true), $savedTimezone) ||
                    !Mage::getResourceModel("analytic/dailyAggregation")
                    ->getIfDateAgrregated($this->getDateTo(true), $savedTimezone)) {
                    if (Mage::getStoreConfig(Analytic_Analytics_Model_DataAggregation::XML_DOWN_CRON_FINISH_FLAG)) {
    ...
    {
        return new Zend_Date(Mage::getResourceModel("analytic/dailyAggregation")
    ...
    {
        return Mage::getResourceModel("analytic/dataAggregation")->isPartitioningAvailable();
    }
}
