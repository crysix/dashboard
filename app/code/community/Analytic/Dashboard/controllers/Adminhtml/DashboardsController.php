...
class Analytic_Dashboard_Adminhtml_DashboardsController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Init action
     * @return Analytic_Dashboard_Adminhtml_DashboardsController
     */
    protected function _initAction()
    {
        $this->loadLayout();
        $this->_setActiveMenu('system/analytic_dashboards');
    ...
    {
        $this->_initAction();
        $this->_addBreadcrumb(Mage::helper('analytic_dashboard')->__('System'), Mage::helper('analytic_dashboard')->__('System'));
        $this->_addBreadcrumb(Mage::helper('analytic_dashboard')->__('Dashboards'), Mage::helper('analytic_dashboard')->__('Dashboards'));
        $this->renderLayout();
    }
    ...
    public function editAction()
    {
        ...
        if ($modelDashboard->getId() || $id == 0) {
            ...
            if (($id == 0) || ($modelDashboard->canEdit($currentUserId))) {
                ...
                $this->_addBreadcrumb(Mage::helper('analytic_dashboard')->__('Dashboard Details'), Mage::helper('analytic_dashboard')->__('Dashboard Details'));

                $this->getLayout()->getBlock('head')->setCanLoadExtJs(true);

                $this->_addContent($this->getLayout()->createBlock('analytic_dashboard/adminhtml_dashboard_edit'))
                    ->_addLeft($this->getLayout()->createBlock('analytic_dashboard/adminhtml_dashboard_edit_tabs'));

                $this->_addJs(
                    $this->getLayout()->createBlock('adminhtml/template')->setTemplate('analytic_dashboard/dashboard_users_js.phtml')
                );

                $this->_addJs(
                    $this->getLayout()->createBlock('adminhtml/template')->setTemplate('analytic_dashboard/dashboard_roles_js.phtml')
                );
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                Mage::getResourceModel('analytic_dashboard/role')->remove($dashboardId);
                foreach ($dashboardRoles as $roleId => $actions) {
                    $modelRole = Mage::getModel('analytic_dashboard/role');
                ...
                Mage::getResourceModel('analytic_dashboard/user')->remove($dashboardId);
        ...
        if (!is_array($dashboardIds)) {
            ...
            try {
                foreach ($dashboardIds as $dashboardId) {
                    $dashboard = Mage::getModel('analytic_dashboard/dashboard')->load($dashboardId);
                    $dashboard->delete();
                }
    ...
    public function userGridAction()
    {
        $this->getResponse()->setBody(
            $this->getLayout()->createBlock('analytic_dashboard/adminhtml_dashboard_edit_tab_users', 'dashboard.users.grid')
                ->toHtml()
        );
    }
    ...
    {
        $this->getResponse()->setBody(
            $this->getLayout()->createBlock('analytic_dashboard/adminhtml_dashboard_edit_tab_roles', 'dashboard.roles.grid')
                ->toHtml()
        );
    }
}
