...
class Analytic_Dashboard_Adminhtml_UserdashboardController extends Mage_Adminhtml_Controller_Action
{
    const WIDGET_WAS_REMOVED_MESSAGE = 'Sorry, widget was probably removed by another user, please refresh the page';

    protected function _construct()
    {
        $this->setUsedModuleName('Analytic_Dashboard');
    }

    /**
     * Init action
     * @return Analytic_Dashboard_Adminhtml_UserdashboardController
     */
     ...
     * @param  Analytic_Dashboard_Model_Dashboard $dashboard
     * @return boolean
     */
    protected function canEdit(Analytic_Dashboard_Model_Dashboard $dashboard)
    {
    ...
    {
        $collection = Mage::getModel('analytic_dashboard/dashboard')->getCollection();
        if (!Mage::helper('analytic_dashboard')->canManageDashboards()) {
    ...
    public function aggregationStartAction()
    {
        ...
        try {
            $data = Mage::getModel('analytic/dataAggregation')->parseDataCronJob(
                array(
                     'goToDown' => false,
    ...
    {
        Mage::helper('analytic_dashboard')->saveTimezoneConfig();
    ...
    public function dailyAggregationProcessAction()
    {
        $result = new Varien_Object();
        $result->setError(false);
        $records = Mage::getModel('analytic/dailyAggregation')->parseDataCronJob(array('goToDown' => true));
    ...
    {
        $defaultDashboard = Mage::getModel("analytic_dashboard/dashboard")->load(Mage::helper('analytic_dashboard')->getDefaultDashboardId());
    ...
    public function dateAction()
    {
        ...
        $storeId = Mage::helper('analytic_dashboard')->getStore();
        $dashboardId = $this->getRequest()->getParam('id');
        if (null === $storeId) {
            $modelDashboard = Mage::getModel('analytic_dashboard/dashboard')->load($dashboardId);
    ...
    public function viewOrPrint()
    {
        ...
        $modelDashboard = Mage::getModel('analytic_dashboard/dashboard')->load($id);
        ...
        if ($modelDashboard->getId()) {
            ...
            if ($modelDashboard->canView($currentUserId)) {
                ...
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('analytic_dashboard')->__("You don't have permissions to view this dashboard"));
                $this->_redirect('*/*/');
            }
        } else {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('analytic_dashboard')->__('Dashboard does not exist'));
            $this->_redirect('*/*/');
        }
    }
    ...
    {
        Mage::helper('analytic_dashboard')->checkTimezoneChange();
        $this->viewOrPrint();
    }
    ...
    public function editAction()
    {
        ...
        if ($modelDashboard->getId() || $id == 0) {
            ...
            if (($id == 0 && $this->canCreate()) || ($this->canEdit($modelDashboard))) {
                ...
                $this->_addContent($this->getLayout()->createBlock('analytic_dashboard/adminhtml_userdashboard_edit'));

                $this->renderLayout();
            } else {
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('analytic_dashboard')->__("You don't have permissions to edit this dashboard"));
                $this->_redirect('*/*/');
            }
        } else {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('analytic_dashboard')->__('Dashboard does not exist'));
    ...
    public function newAction()
    {
        if ($this->canCreate()) {
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('analytic_dashboard')->__("You don't have permissions to create new dashboard"));
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost()) {
            ...
            $modelDashboard = Mage::getModel('analytic_dashboard/dashboard');
            $modelDashboard->setData($data)
                ->setId($dashboardId);
            if (!$dashboardId) {
            ...
            try {
                if (($dashboardId == 0 && $this->canCreate()) || ($this->canEdit($modelDashboard))) {
                    ...
                    $defaultDashboardId = Mage::helper('analytic_dashboard')->getDefaultDashboardId();
                    $userId = Mage::getSingleton('admin/session')->getUser()->getId();

                    if ($dashboardId == 0) {
                        /*Saving permissions for current user*/
                        $modelUser = Mage::getModel('analytic_dashboard/user');
                    ...
                    if (isset($data['is_default'])) {
                        Mage::getResourceModel('analytic_dashboard/user')->setDefault($modelDashboard->getId(), $userId);
                    } else {
                        /* Unset Default Dashboard if checkbox was unchecked*/
                        if ($defaultDashboardId == $modelDashboard->getId()) {
                            Mage::getResourceModel('analytic_dashboard/user')->setNotDefault($modelDashboard->getId(), $userId);
                        }
                    }

                    Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('analytic_dashboard')->__('Dashboard was successfully saved'));
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('analytic_dashboard')->__('Unable to find dashboard to save'));
        $this->_redirect('*/*/');
    }
    ...
    public function savewidgetAction()
    {
        ...
        if ($data = $this->getRequest()->getPost()) {
            $dashboardId = $this->getRequest()->getParam('id');
            if ($this->canEdit(Mage::getModel('analytic_dashboard/dashboard')->load($dashboardId))) {
                $widgetId = $this->getRequest()->getParam('widget_id');
                if (!$widgetId) {
                    $widgetId = null;
                }
                /*Collecting Widget Data*/
                $modelWidget = Mage::getModel('analytic_dashboard/widget');
                ...
                try {
                    ...
                    $html = $this->getLayout()->createBlock('analytic_dashboard/adminhtml_userdashboard_widget_' . $modelWidget->getRawWidgetConfig()->getType())
        ...
        try {
            if ($data = $this->getRequest()->getPost()) {
                ...
                if ($this->canEdit(Mage::getModel('analytic_dashboard/dashboard')->load($dashboardId))) {
                    ...
                    $widgetResource = Mage::getResourceModel("analytic_dashboard/widget");
    ...
    public function loadwidgetconfigAction()
    {
        ...
        try {
            $widgetId = $this->getRequest()->getParam('widget_id');
            $widget = Mage::getModel("analytic_dashboard/widget")->load($widgetId);
            ...
            $html = $this->getLayout()->createBlock('analytic_dashboard/adminhtml_userdashboard_tab_' . $tab)
                ->setWidget($widget)
                ->toHtml();
            $result->setTab($tab);
            $result->setHtml($html);
    ...
    public function deletewidgetAction()
    {
        ...
        if ($this->getRequest()->getParam('widget_id') > 0) {
            ...
            try {
                if ($this->canEdit(Mage::getModel('analytic_dashboard/dashboard')->load($dashboardId))) {
                    $model = Mage::getModel('analytic_dashboard/widget');
    ...
    public function deleteAction()
    {
        if ($this->getRequest()->getParam('id') > 0) {
            try {
                $model = Mage::getModel('analytic_dashboard/dashboard');
