...
class Analytic_Dashboard_Block_Adminhtml_Userdashboard extends Mage_Adminhtml_Block_Template
{
    protected $_objectId;
    ...
    protected function _prepareLayout()
    {
        $this->setChild('edit_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')
                ->setData(array(
                'label' => Mage::helper('analytic_dashboard')->__('Edit Details'),
        ...
        $this->setChild('create_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')
                ->setData(array(
                'label' => Mage::helper('analytic_dashboard')->__('Add Dashboard'),
        ...
        if ($this->getDashboards()->count() > 1) {
            $this->setChild('delete_button',
                $this->getLayout()->createBlock('adminhtml/widget_button')
                    ->setData(array(
                    'label' => Mage::helper('analytic_dashboard')->__('Delete'),
        ...
        $this->setChild('widget_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')
                ->setData(array(
                'label' => Mage::helper('analytic_dashboard')->__('Add Widget'),
                'onclick' => "addWidget()",
                'class' => 'add'
            )));
        ...
        $this->setChild('print_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')
                ->setData(array(
                'label' => Mage::helper('analytic_dashboard')->__('Print Page'),
    ...
    {
        return $this->getUrl('*/*/print', array($this->_objectId => $this->getCurrentId(),"store" => Mage::helper('analytic_dashboard')->getStore()));
    }
    ...
    public function canEdit()
    {
        if ($currentDashboard = $this->getCurrentDashboard()) {
            $modelDashboard = Mage::getModel('analytic_dashboard/dashboard')->load($currentDashboard->getId());
    ...
    {
        $params = array('id' => $dashboardId, 'store' => Mage::getModel('analytic_dashboard/dashboard')->load($dashboardId)->getDefaultStoreId());
    ...
    {
        $params = array('id' => $dashboardId, 'store' => Mage::helper('analytic_dashboard')->getStore());
     ...
     *
     * @return Analytic_Dashboard_Model_Resource_Dashboard_Collection
     */
    ...
    {
        $collection = Mage::getModel('analytic_dashboard/widget')->getCollection();
     ...
     * @param  Analytic_Dashboard_Model_Widget $widget
     * @return string
     */
    public function getTabContent($tabName, $widget = null)
    {
        $html = $this->getLayout()->createBlock('analytic_dashboard/adminhtml_userdashboard_tab_' . $tabName)->toHtml();
    ...
    {
        $ranges = array();
        $ranges[] = array('label' => $this->__('Today'), 'value' => $this->helper('analytic_dashboard')->getDateRange('today'));
        $ranges[] = array('label' => $this->__('Yesterday'), 'value' => $this->helper('analytic_dashboard')->getDateRange('yesterday'));
        $ranges[] = array('label' => $this->__('Last Week'), 'value' => $this->helper('analytic_dashboard')->getDateRange('last week'));
        $ranges[] = array('label' => $this->__('Last Month'), 'value' => $this->helper('analytic_dashboard')->getDateRange('last month'));
    ...
    {
        $isSelected = false;
        $currentRange = $this->helper('analytic_dashboard')->getDateFrom() . '#' . $this->helper('analytic_dashboard')->getDateTo();
