...
class Analytic_Dashboard_Block_Adminhtml_Userdashboard_Edit_Form extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        $form = new Varien_Data_Form();

        $fieldset = $form->addFieldset('dashboard_form', array('legend' => Mage::helper('analytic_dashboard')->__('Dashboard Details')));

        $fieldset->addField('name', 'text', array(
            'label' => Mage::helper('analytic_dashboard')->__('Name'),
            'required' => true,
            'name' => 'name',
        ));

        $fieldset->addField('description', 'textarea', array(
            'label' => Mage::helper('analytic_dashboard')->__('Description'),
            'required' => false,
            'name' => 'description',
        ));

        /*$fieldset->addField('layout', 'select', array(
            'label' => Mage::helper('analytic_dashboard')->__('Layout'),
            'required' => true,
            'name' => 'layout',
            'values' => Mage::helper('analytic_dashboard')->getLayoutOptions()
        ));*/
        ...
        if (count(Mage::app()->getStores()) > 1) {
            $fieldset->addField('default_store_id', 'select', array(
                'label' => Mage::helper('analytic_dashboard')->__('Default Store'),
                'required' => false,
        ...
        $fieldset->addField('is_default', 'checkbox', array(
            'label' => Mage::helper('analytic_dashboard')->__('Is Default'),
            'required' => false,
            'name' => 'is_default',
            'checked' => (Mage::helper('analytic_dashboard')->getDefaultDashboardId() == $this->getRequest()->getParam('id'))
        ));
